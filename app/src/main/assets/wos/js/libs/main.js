    ...
    if (localStorage.getItem('PopupNotice') === null) {
        ...
        events.forEach(function (event) {
            if (event.PopupNotice != null) {
                pn.push({ 'Id': event.Id.toString(), 'GUID': '' });
            }
        });
    }
    else {
...
var showPopupNotice = function (eventId) {
    if (localStorage.getItem('PopupNotice') !== null) {
        ...
        data.forEach(function (pn) {
            if (Number(pn.Id) === Number(eventId) && pn.GUID !== event.PopupNotice.GUID) {
                var ok = false;
                if (!ok) {
                ...
                if (ok) {
                    ...
                    pn.GUID = event.PopupNotice.GUID;
                }
            }
        });
...
var checkAdhocMessage = function () {
    if (__deviceInfo.OS.toLowerCase() === "android") {
        ...
        webkit.messageHandlers.checkAdhocMessage.postMessage('');
    } else {
        console.log("Call app check adhoc message");
        doAfterCheckAdhocMessage('');
    }
};
